...
try:
    ...
    # Initialize bins
    bin_edges = np.linspace(0, max_depth, num_bins + 1)  # e.g., [0, 0.314, 0.428, ..., 2.2]
    print("\nBin edges:", bin_edges)
    ...
    cone_probe_count = 0
    first_bin_probes = []  # To store details of probes in the first bin
    for idx, row in vital_data.iterrows():
        ...
        if x_gt >= min_x_gt and abs(y_gt) <= x_gt * np.tan(half_fov_rad):
            ...
            for i in range(num_bins):
                ...
                print(f"Probe at index {idx} with x_gt={x_gt} not assigned to any bin.")

    # Print diagnostics for first bin
    print(f"\nDetails of probes in first bin (Bin 0: {bin_edges[0]:.3f}-{bin_edges[1]:.3f} m):")
    for probe in first_bin_probes:
        print(f"Index {probe['index']}: x_gt={probe['x_gt']:.6f}, y_gt={probe['y_gt']:.6f}, error={probe['error']:.6f}")
    print(f"Total probes in first bin: {len(first_bin_probes)}")
    ...
    cone_probes = vital_data[
        (vital_data['x_gt'] >= min_x_gt) &
        (np.abs(vital_data['y_gt']) <= vital_data['x_gt'] * np.tan(half_fov_rad))
    ]
    ax.scatter(cone_probes['y_gt'], cone_probes['x_gt'], c='red', s=10, label='Probes in Cone')

    # Set plot limits to include all probes
    x_min = min(vital_data['y_gt'].min(), 0)
    x_max = max(vital_data['y_gt'].max(), max_depth)
